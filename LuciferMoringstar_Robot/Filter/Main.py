    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                 [
                    InlineKeyboardButton(text=f"{get_size(file.file_size)}",callback_data=f"https://telegram.dog/{nyva}?start=pr0fess0r_99_-_-_-_{file_id}"),
                    InlineKeyboardButton(text=f"{file.file_name}",callback_data=f"https://telegram.dog/{nyva}?start=pr0fess0r_99_-_-_-_{file_id}") 
                 ]
            )
