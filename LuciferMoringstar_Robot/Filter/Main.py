    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                 [
                    InlineKeyboardButton(f"{get_size(file.file_size)},callback_data=f"pr0fess0r_99#{file_id}")
                    InlineKeyboardButton(f"{file.file_name},callback_data=f"pr0fess0r_99#{file_id}") 
                 ]
            )
